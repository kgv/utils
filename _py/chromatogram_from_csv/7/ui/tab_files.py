...
class TabFiles(ttk.Frame):
    ...
    def update_list(self, plots: List[PlotItem]) -> None:
        ...
        for i, item in enumerate(reversed(plots)):
            ...
            sp.config(command=lambda ix=idx, s=sp: self.controller.update_plot(ix, 'line_width', s.get()))
            sp.delete(0, tk.END)
            ...
            sp_x.config(command=lambda ix=idx, s=sp_x: self.controller.update_plot(ix, 'x_offset', s.get()))
            ...
            sp_y.config(command=lambda ix=idx, s=sp_y: self.controller.update_plot(ix, 'y_offset', s.get()))
