    ...
    def update_list(self, helpers: List[HelperLine], x_step: float, y_step: float) -> None:
        ...
        for i, h in enumerate(helpers):
            ...
            for k, w, step, tip in [
                ...
                sp.config(command=lambda ix=i, key=k, s=sp: self.controller.update_helper(ix, key, s.get()))
                sp.delete(0, tk.END)
            ...
            sp_p.config(command=lambda ix=i, s=sp_p: self.controller.update_helper(ix, 'label_pos', s.get()))
            ...
            for k, w, step, tip in [
                ...
                sp_o.config(command=lambda ix=i, key=k, s=sp_o: self.controller.update_helper(ix, key, s.get()))
            ...
            sp_w.config(command=lambda ix=i, s=sp_w: self.controller.update_helper(ix, 'width', s.get()))
