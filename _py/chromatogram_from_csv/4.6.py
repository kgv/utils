...
class ChromatogramApp:
    ...
    def refresh_plots(self, force=False):
        ...
        if self.mode_var.get() == "Нормированный":
            for it in self.plots_registry:
                s_t, e_t = self.scale_min.get(), self.scale_max.get() # Re-read current limits
    ...
    def setup_tab_files(self):
        ...
        # ИСПРАВЛЕНО: Порядок заголовков (Смещ. X теперь перед Легендой)
        cols = [("Слой", 60), ("Цвет", 40), ("Стиль", 50), ("Толщ", 50), ("Смещ. X", 60), ("Лег", 40), ("Имя в легенде", 200)]
    ...
    def add_files(self):
        ...
        for p in paths:
            ...
            if t_raw is not None:
                ...
                # Применяем перевод в минуты и смещение
                ...
                self.plots_registry.append({
                    ...
                    'show_in_legend': True,
                    'x_offset': 0.0  # ДОБАВЛЕНО: Индивидуальное смещение
                })
    ...
    def update_ui_list(self):
        ...
        for i, item in enumerate(reversed(self.plots_registry)):
            ...
            # 5. СМЕЩЕНИЕ ПО X (Spinbox) - Ячейка для настройки
    ...
    def edit_plot(self, idx, key, val):
        try:
            if key == 'line_width':
                ...
                val = float(raw_val)
            elif key == 'x_offset': # ДОБАВЛЕНО: Обработка индивидуального смещения
            ...
            elif key == 'x_offset': # ДОБАВЛЕНО: Пересчет при изменении смещения
                self.reapply_offset_to_all_plots()
