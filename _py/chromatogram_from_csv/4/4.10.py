...
class ChromatogramApp:
    ...
    def refresh_plots(self, force=False):
        ...
        # ИЗМЕНЕНО: Логика применения Y с учетом автомасштаба
                ...
                if self.plots_registry:
                    ...
                    for it in self.plots_registry:
                        if it.get('visible', True) and it['y_orig'].size > 0:
                            vis_max = max(vis_max, it['y_orig'].max())
                    
                    if vis_max > 0:
                        y_min, y_max = -vis_max * 0.02, vis_max * 1.05
                    else:
                        y_min, y_max = 0, 1
                else:
                    all_y_coords = []
    ...
    def setup_tab_files(self):
        ...
        # ДОБАВЛЕНО: Колонка "Вид"
        cols = [("Слой", 60), ("Вид", 40), ("Цвет", 40), ("Стиль", 50), ("Толщ", 50), ("Смещ. X", 60), ("Лег", 40), ("Имя в легенде", 200)]
        ...
        for p in paths:
            ...
            if t_raw is not None:
                ...
                self.plots_registry.append({
                    ...
                    'show_in_legend': True,
                    'x_offset': 0.0,
                    'visible': True  # ДОБАВЛЕНО: Флаг видимости
                })
    ...
    def edit_plot(self, idx, key, val):
        try:
            ...
            if key == 'linestyle': 
                ...
                self.reapply_offset_to_all_plots()
            elif key == 'visible': # ДОБАВЛЕНО: Скрытие/показ линии
    ...
    def manual_fit(self):
        if self.mode_var.get() == "Абсолютный":
            ...
            if m > 0: 
                # ИЗМЕНЕНО: Устанавливаем значения в поля и отключаем автомасштаб
                self.y_min_var.set(-m * 0.02)
                self.y_max_var.set(m * 1.05)
