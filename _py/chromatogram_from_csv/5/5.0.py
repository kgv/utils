...
class ChromatogramApp:
    ...
    def auto_label_pos(self, idx):
        h = self.helper_lines[idx]
        max_val = float('-inf')
        
        for p in self.plots_registry:
            if not p.get('visible', True):
                continue
                
            x_data = p['line'].get_xdata()
            y_data = p['line'].get_ydata()
            
            if len(x_data) == 0 or len(y_data) == 0:
                continue
                
            if h['type'] == 'V':
                # Ищем высоту Y именно в той точке X, где стоит вертикальная линия
                idx_closest = np.argmin(np.abs(x_data - h['pos']))
                max_val = max(max_val, y_data[idx_closest])
            else:
                # Для горизонтальной линии ищем X в той точке Y, где стоит линия
                idx_closest = np.argmin(np.abs(y_data - h['pos']))
                max_val = max(max_val, x_data[idx_closest])
        
        if max_val != float('-inf'):
            # Добавляем небольшой отступ (2% от видимого диапазона), 
            # чтобы текст аккуратно висел над графиком, а не перекрывал его
            if h['type'] == 'V':
                y_lim = self.ax.get_ylim()
                padding = abs(y_lim[1] - y_lim[0]) * 0.02
            else:
                x_lim = self.ax.get_xlim()
                padding = abs(x_lim[1] - x_lim[0]) * 0.02
                
            h['label_pos'] = round(max_val + padding, 2)
            
            # Обновляем значение в интерфейсе
            if 'label_pos_spin' in h:
                h['label_pos_spin'].delete(0, "end")
                h['label_pos_spin'].insert(0, str(h['label_pos']))
            self.refresh_plots(force=True)
    ...
    def create_control_window(self):
        ...
        ttk.Button(bottom_panel, text="💾 Сохранить график", 
                   command=self.save_plot).pack(side="left", expand=True, fill="x", padx=5)
    ...
    def setup_tab_style(self):
        ...
        # ДОБАВЛЕНО: Настройка размера графика
        ...
        ent_w.bind("<Return>", self.apply_figure_size)
        ent_w.bind("<FocusOut>", self.apply_figure_size)
        ...
        ent_h.bind("<Return>", self.apply_figure_size)
        ent_h.bind("<FocusOut>", self.apply_figure_size)
        ...
        self.preset_cb = ttk.Combobox(size_frame, values=[
            ...
            "FullHD (19.2x10.8)", 
            "4K (38.4x21.6)", 
            "A4 Альбомная (11.7x8.3)",
            "Квадрат (10x10)",
            "Свой размер"
        ], state="readonly", width=22)
        self.preset_cb.set("По умолчанию (10x6)")
        self.preset_cb.pack(side="left")
        self.preset_cb.bind("<<ComboboxSelected>>", self.apply_figure_preset)
    ...
    # Методы для применения размера графика
    def apply_figure_size(self, event=None):
        try:
            w = float(self.fig_w_var.get())
            h = float(self.fig_h_var.get())
            self.fig.set_size_inches(w, h)
            
            # ИЗМЕНЕНО: Меняем физический размер главного окна (дюймы * DPI)
            ...
            # Если метод вызван вручную из поля ввода (есть event), ставим "Свой размер"
            if event is not None:
                self.preset_cb.set("Свой размер")
        except ValueError:
            pass
        self.root.focus_set()

    def apply_figure_preset(self, event=None):
        preset = self.preset_cb.get()
        if "HD" in preset and "FullHD" not in preset: w, h = 12.8, 7.2
        elif "FullHD" in preset: w, h = 19.2, 10.8
        elif "4K" in preset: w, h = 38.4, 21.6
        elif "A4" in preset: w, h = 11.69, 8.27
        elif "Квадрат" in preset: w, h = 10.0, 10.0
        elif "Свой размер" in preset: return
        else: w, h = 10.0, 6.0
        
        self.fig_w_var.set(w)
        self.fig_h_var.set(h)
        
        # ИЗМЕНЕНО: Вызываем единый метод применения размера
        self.apply_figure_size(None)
    ...
    def refresh_plots(self, force=False):
        ...
        if not self.plots_registry and not self.helper_lines: return
        
        if self.plots_registry:
            s_t, e_t = self.scale_min.get(), self.scale_max.get()
            e_t_safe = e_t if e_t > s_t else s_t+0.01
            if self.reverse_x_var.get():
                self.ax.set_xlim(e_t_safe, s_t)
            else:
                self.ax.set_xlim(s_t, e_t_safe)
        elif self.helper_lines:
            all_x_coords = []
            for h in self.helper_lines:
                if h['type'] == 'V':
                    ...
                    ty = h['label_pos'] # ИЗМЕНЕНО: Берем координату напрямую
                    ...
                    tx = h['label_pos'] # ИЗМЕНЕНО: Берем координату напрямую
                    t = self.ax.text(tx, ty, txt, color=h['color'], fontsize=h['font_size'], 
                                    va='center', ha='center', fontweight='bold', zorder=z+1, 
                                    rotation=rot, rotation_mode='anchor')
            if all_x_coords:
                ...
                if self.reverse_x_var.get():
                    self.ax.set_xlim(max_x + padding, min_x - padding)
                else:
                    self.ax.set_xlim(min_x - padding, max_x + padding)
            else:
                self.ax.set_xlim(1, 0) if self.reverse_x_var.get() else self.ax.set_xlim(0, 1)
        else:
            self.ax.set_xlim(1, 0) if self.reverse_x_var.get() else self.ax.set_xlim(0, 1)
        ...
        if self.mode_var.get() == "Нормированный":
            for it in self.plots_registry:
                ...
                v_y = it['y_orig'][mask]
                base_y = (it['y_orig'] / v_y.max() * 100) if v_y.size and v_y.max()>0 else it['y_orig']*0
                it['line'].set_ydata(base_y + it.get('y_offset', 0.0))
            
            if getattr(self, 'auto_y_var', None) and self.auto_y_var.get():
                if self.plots_registry:
                    v_min, v_max = float('inf'), float('-inf')
                    for it in self.plots_registry:
                        if it.get('visible', True) and it['y_orig'].size > 0:
                            y_data = it['line'].get_ydata()
                            v_min = min(v_min, y_data.min())
                            v_max = max(v_max, y_data.max())
                    if v_max != float('-inf'):
                        yr = v_max - v_min if v_max > v_min else 100
                        self.ax.set_ylim(v_min - yr*0.02, v_max + yr*0.05)
                        self.y_min_var.set(v_min - yr*0.02)
                        self.y_max_var.set(v_max + yr*0.05)
                    else:
                        self.ax.set_ylim(-2, 105)
                        self.y_min_var.set(-2)
                        self.y_max_var.set(105)
                else:
                    self.ax.set_ylim(-2, 105)
                    self.y_min_var.set(-2)
                    self.y_max_var.set(105)
            ...
            for it in self.plots_registry: 
                it['line'].set_ydata(it['y_orig'] + it.get('y_offset', 0.0))
            ...
            if getattr(self, 'auto_y_var', None) and self.auto_y_var.get():
                if self.plots_registry:
                    ...
                    if v_max != float('-inf'):
                        yr = v_max - v_min if v_max > v_min else (v_max if v_max > 0 else 1)
                        y_min, y_max = v_min - yr * 0.02, v_max + yr * 0.05
                    else:
                        y_min, y_max = 0, 1
                else:
                    all_y_coords = []
        ...
        for it in self.plots_registry: 
            show_leg = it['show_in_legend'] and it.get('visible', True)
            it['line'].set_label(it['label'] if show_leg else "_nolegend_")
        ...
        for h in self.helper_lines:
            ...
            rot = h.get('label_rotation', 0)
            
            if h['type'] == 'V':
                l = self.ax.plot([h['pos'], h['pos']], [h['start'], h['end']], color=h['color'], lw=h['width'], zorder=z)[0]
                tx = h['pos'] + h.get('label_offset', 0)
                ty = h['start'] + (h['end'] - h['start']) * h['label_pos']
                # ИЗМЕНЕНО: Добавлен rotation_mode='anchor'
                t = self.ax.text(tx, ty, txt, color=h['color'], fontsize=h['font_size'], 
                                 va='center', ha='center', fontweight='bold', zorder=z+1, 
                                 rotation=rot, rotation_mode='anchor')
            else:
    ...
    def setup_tab_files(self):
        ...
        # ИЗМЕНЕНО: Добавлена колонка "Смещ. Y"
        cols = [("Слой", 60), ("Вид", 40), ("Цвет", 40), ("Стиль", 50), ("Толщ", 50), ("Смещ. X", 60), ("Смещ. Y", 60), ("Лег", 40), ("Имя в легенде", 200)]
    ...
    def setup_tab_scale(self):
        ...
        # Чекбокс инверсии оси X
        ttk.Checkbutton(frame, text="Инвертировать ось X (справа налево)", 
                        variable=self.reverse_x_var, 
                        command=self.refresh_plots).pack(anchor="w", pady=(5, 0))
        
        offset_frame = ttk.Frame(frame); offset_frame.pack(fill="x", pady=(10, 5))
        ...
        self.offset_entry.bind("<Return>", self.on_offset_entry)
        self.offset_entry.bind("<FocusOut>", self.on_offset_entry)
        ...
        # ДОБАВЛЕНО: Настройки оси Y
        ttk.Separator(frame, orient="horizontal").pack(fill="x", pady=15)
        ttk.Label(frame, text="Ось Y (Интенсивность):", font='Arial 10 bold').pack(anchor="w")
        
        if not hasattr(self, 'auto_y_var'):
            self.auto_y_var = tk.BooleanVar(value=True)
            self.y_min_var = tk.DoubleVar(value=0.0)
            self.y_max_var = tk.DoubleVar(value=100.0)
            
        ttk.Checkbutton(frame, text="Автомасштаб Y", variable=self.auto_y_var, command=self.refresh_plots).pack(anchor="w", pady=(5, 5))
        
        y_frame = ttk.Frame(frame); y_frame.pack(fill="x")
        ...
        self.ent_y_min.bind("<Return>", lambda e: self.apply_manual_y())
        self.ent_y_min.bind("<FocusOut>", lambda e: self.apply_manual_y())
        ...
        self.ent_y_max.bind("<Return>", lambda e: self.apply_manual_y())
        self.ent_y_max.bind("<FocusOut>", lambda e: self.apply_manual_y())

        ttk.Button(frame, text="🔍 Подогнать Y по видимому X", command=self.manual_fit).pack(fill="x", pady=15)

    # Метод для применения ручных значений Y
    def apply_manual_y(self):
        try:
            min_val = float(self.ent_y_min.get().replace(',', '.'))
            max_val = float(self.ent_y_max.get().replace(',', '.'))
            self.y_min_var.set(min_val)
            self.y_max_var.set(max_val)
            self.auto_y_var.set(False) # Отключаем автомасштаб при ручном вводе
            self.refresh_plots(force=True)
        except ValueError:
            pass
        self.root.focus_set()
    ...
    def on_time_unit_change(self):
        self.reapply_offset_to_all_plots()
        self.refresh_plots(force=True)

    def on_offset_entry(self, event=None):
        try:
            new_offset = float(self.offset_entry.get().replace(',', '.').strip())
            if new_offset != self.time_offset:
                self.time_offset = new_offset
                # Re-load all data to apply the new offset
                self.reapply_offset_to_all_plots()
                self.refresh_plots(force=True)
        except ValueError:
            # If invalid input, revert entry to current offset
            self.time_offset_var.set(self.time_offset)
        self.root.focus_set() # Remove focus from entry

    def reapply_offset_to_all_plots(self):
        factor = 60.0 if self.convert_sec_to_min_var.get() else 1.0
        new_all_data_bounds = [float('inf'), float('-inf')]
        
        for item in self.plots_registry:
            if 't_raw' not in item:
                item['t_raw'] = item['t'] - self.time_offset
            
            # ИЗМЕНЕНО: Применяем перевод в минуты, глобальное смещение и индивидуальное смещение
            indiv_offset = item.get('x_offset', 0.0)
            item['t'] = (item['t_raw'] / factor) + self.time_offset + indiv_offset
            item['line'].set_xdata(item['t'])

            new_all_data_bounds[0] = min(new_all_data_bounds[0], item['t'].min())
            new_all_data_bounds[1] = max(new_all_data_bounds[1], item['t'].max())
        
        self.all_data_bounds = new_all_data_bounds
        if self.all_data_bounds[0] != float('inf'):
            ...
            if self.scale_min.get() < self.all_data_bounds[0]:
                self.scale_min.set(self.all_data_bounds[0])
            if self.scale_max.get() > self.all_data_bounds[1]:
                self.scale_max.set(self.all_data_bounds[1])
        else:
            self.scale_min.config(from_=0, to=1)
            self.scale_max.config(from_=0, to=1)
            self.scale_min.set(0)
            self.scale_max.set(1)
    ...
    def add_helper(self):
        ...
        new_h = {
            ...
            'width': 0.5,
            'text': '', 
            'label_pos': round(y_l[1] if is_v else x_l[1], 2), # ИЗМЕНЕНО: Теперь это абсолютная координата (по умолчанию верх/право)
            'label_offset': 0.0, 
            'label_rotation': 0,
            'layer': 'Задний', 
            'font_size': 9
    ...
    def update_helper_ui(self):
        ...
        for i, h in enumerate(self.helper_lines):
            ...
            # Позиция текста (абсолютная координата)
            sp_p = tk.Spinbox(f_row, from_=-1e12, to=1e12, width=7, command=lambda ix=i: self.edit_helper_spin(ix, 'label_pos'))
            sp_p.delete(0, "end"); sp_p.insert(0, str(h['label_pos'])); sp_p.pack(side="left", padx=1)
            ...
            ToolTip(sp_p, "Координата текста (Y для вертикальной, X для горизонтальной)")

            # Кнопка "Авто"
            btn_auto = ttk.Button(f_row, text="Авто", width=4, command=lambda ix=i: self.auto_label_pos(ix))
    ...
    def update_helper_increments(self):
        ...
        for h in self.helper_lines:
            try:
                ...
                if step_main == 0: step_main = 0.1
                if step_range == 0: step_range = 0.1
                ...
                if f'label_pos_spin' in h: h['label_pos_spin'].config(increment=step_range)
                if f'label_rotation_spin' in h: h['label_rotation_spin'].config(increment=15) # ДОБАВЛЕНО: Шаг поворота 15 градусов
            except Exception as e:
                pass

    def edit_helper(self, idx, key, val):
        try:
            # ИЗМЕНЕНО: Добавили label_pos в общий список координат
            if key in ['pos', 'start', 'end', 'label_offset', 'label_rotation', 'label_pos']:
                ...
                clean_val = str(val).replace(',', '.').strip()
                if not clean_val: return
                self.helper_lines[idx][key] = int(float(clean_val))
            elif key == 'width':
                clean_val = str(val).replace(',', '.').strip()
                if not clean_val: return
                self.helper_lines[idx][key] = float(clean_val)
        ...
        try:
            ...
            if not raw_val:
                if key == 'font_size':
                    ...
                    spinbox_widget.insert(0, str(int(self.helper_lines[idx].get(key, 9))))
                else:
                    spinbox_widget.delete(0, "end")
                    spinbox_widget.insert(0, str(self.helper_lines[idx].get(key, 0)))
                return
    ...
    def add_files(self):
        ...
        for p in paths:
            t_raw, y = self.load_data(p)
            if t_raw is not None:
                lbl = os.path.basename(p).split('.')[0]
                color = self.palette[len(self.plots_registry) % len(self.palette)]
                
                t_with_offset = (t_raw / factor) + self.time_offset
                line, = self.ax.plot(t_with_offset, y, label=lbl, lw=self.style_params['line_width'], color=color)
                
                self.plots_registry.append({
                    't_raw': t_raw, 
                    't': t_with_offset, 
                    'y_orig': y, 'line': line, 'color': color, 
                    'label': lbl, 'linestyle': '-', 'line_width': self.style_params['line_width'], 
                    'show_in_legend': True,
                    'x_offset': 0.0,
                    'y_offset': 0.0,  # ДОБАВЛЕНО: Смещение по Y
                    'visible': True
                })
                self.all_data_bounds = [min(self.all_data_bounds[0], t_with_offset.min()), max(self.all_data_bounds[1], t_with_offset.max())]
    ...
    def update_ui_list(self):
        ...
        for i, item in enumerate(reversed(self.plots_registry)):
            ...
            # 2. Видимость (Чекбокс)
            vis_v = tk.BooleanVar(value=item.get('visible', True))
            ttk.Checkbutton(f, variable=vis_v, command=lambda ix=idx, v=vis_v: self.edit_plot(ix, 'visible', v.get())).pack(side="left", padx=7)
            ...
            # 6. СМЕЩЕНИЕ ПО X
            sp_x = tk.Spinbox(f, from_=-1e9, to=1e9, increment=0.1, width=5, command=lambda ix=idx: self.edit_plot(ix, 'x_offset', None))
            sp_x.delete(0, "end"); sp_x.insert(0, f"{item.get('x_offset', 0.0):.2f}"); sp_x.pack(side="left", padx=2)
            item['x_offset_spin'] = sp_x
            sp_x.bind("<Return>", lambda e, ix=idx: [self.edit_plot(ix, 'x_offset', None), self.root.focus_set()])
            sp_x.bind("<FocusOut>", lambda e, ix=idx: self.edit_plot(ix, 'x_offset', None))
            
            # ДОБАВЛЕНО: 6.5 СМЕЩЕНИЕ ПО Y
            sp_y = tk.Spinbox(f, from_=-1e12, to=1e12, increment=10, width=6, command=lambda ix=idx: self.edit_plot(ix, 'y_offset', None))
            sp_y.delete(0, "end"); sp_y.insert(0, f"{item.get('y_offset', 0.0):.2f}"); sp_y.pack(side="left", padx=2)
            item['y_offset_spin'] = sp_y
            sp_y.bind("<Return>", lambda e, ix=idx: [self.edit_plot(ix, 'y_offset', None), self.root.focus_set()])
            sp_y.bind("<FocusOut>", lambda e, ix=idx: self.edit_plot(ix, 'y_offset', None))
            ...
            # 8. Кнопка удаления
            ttk.Button(f, text="🗑️", width=3, command=lambda ix=idx: self.remove_plot(idx)).pack(side="right", padx=2)
            
            # 9. Название в легенде
    ...
    def remove_plot(self, idx):
        ...
        if not self.plots_registry:
            ...
            # Recalculate all_data_bounds based on remaining plots
            new_min = float('inf')
            new_max = float('-inf')
            for p_item in self.plots_registry:
                new_min = min(new_min, p_item['t'].min())
                new_max = max(new_max, p_item['t'].max())
            self.all_data_bounds = [new_min, new_max]
            
            self.recalculate_z_orders() # Обновляем слои оставшихся
            
        # Update scale sliders and entries based on new bounds
        if self.all_data_bounds[0] != float('inf'):
            self.scale_min.config(from_=self.all_data_bounds[0], to=self.all_data_bounds[1])
            self.scale_max.config(from_=self.all_data_bounds[0], to=self.all_data_bounds[1])
            # Ensure current slider values are within new bounds
            if self.scale_min.get() < self.all_data_bounds[0]:
                self.scale_min.set(self.all_data_bounds[0])
            if self.scale_max.get() > self.all_data_bounds[1]:
                self.scale_max.set(self.all_data_bounds[1])
    ...
    def edit_plot(self, idx, key, val):
        try:
            if key == 'line_width':
                ...
                raw_val = self.plots_registry[idx]['x_offset_spin'].get().replace(',', '.').strip()
                if not raw_val: return
                val = float(raw_val)
            elif key == 'y_offset': # ДОБАВЛЕНО
                raw_val = self.plots_registry[idx]['y_offset_spin'].get().replace(',', '.').strip()
                if not raw_val: return
                val = float(raw_val)
            ...
            if key == 'linestyle': 
                ...
                self.plots_registry[idx]['line'].set_visible(val)
    ...
    def manual_fit(self):
        if self.mode_var.get() == "Абсолютный":
            s, e = self.scale_min.get(), self.scale_max.get()
            m_min, m_max = float('inf'), float('-inf')
            for it in self.plots_registry:
                if not it.get('visible', True):
                    continue
                
                # ИЗМЕНЕНО: Учитываем смещение по Y
                v = it['y_orig'][(it['t'] >= s) & (it['t'] <= e)] + it.get('y_offset', 0.0)
                if v.size: 
                    m_min = min(m_min, v.min())
                    m_max = max(m_max, v.max())
                    
            if m_max != float('-inf'): 
                yr = m_max - m_min if m_max > m_min else (m_max if m_max > 0 else 1)
                self.y_min_var.set(m_min - yr * 0.02)
                self.y_max_var.set(m_max + yr * 0.05)
    ...
    def load_data(self, p):
        try:
            ...
            # 1. Ищем частоту и начало данных
            for i, line in enumerate(lines):
                if "Частота" in line:
                    parts = line.split(';')
                    if len(parts) > 1:
                        freq = float(parts[1].replace(',', '.').strip())
                
                # Данные начинаются там, где строка начинается с цифры
    ...
    def save_plot(self):
        p = filedialog.asksaveasfilename(
            ...
            filetypes=[("PNG изображение", "*.png"), ("SVG вектор", "*.svg"), ("Все файлы", "*.*")]
        )
        if p:
            ext = p.split('.')[-1].lower()
            if ext == 'png':
                # Для PNG задаем высокое разрешение (dpi=300) для профессионального качества
                self.fig.savefig(p, format='png', bbox_inches='tight', dpi=300)
            else:
                self.fig.savefig(p, format='svg', bbox_inches='tight')

    def on_close(self): plt.close('all'); self.root.quit(); self.root.destroy()

    # Сохранение размера в стиль
    def export_style(self):
        ...
        if path:
            ...
            data['time_offset'] = self.time_offset_var.get()
            data['convert_sec_to_min'] = self.convert_sec_to_min_var.get()
            data['reverse_x'] = self.reverse_x_var.get()
            
            # Сохраняем размеры
            data['fig_width'] = self.fig_w_var.get()
            data['fig_height'] = self.fig_h_var.get()
            
            with open(path, 'w', encoding='utf-8') as f: json.dump(data, f, indent=4, ensure_ascii=False)

    # Загрузка размера из стиля
    def import_style(self):
        ...
        if path:
            try:
                ...
                if 'convert_sec_to_min' in new_s:
                    self.convert_sec_to_min_var.set(new_s['convert_sec_to_min'])
                if 'reverse_x' in new_s:
                    self.reverse_x_var.set(new_s['reverse_x'])
                if 'time_offset' in new_s:
                    self.time_offset = new_s['time_offset']
                    self.time_offset_var.set(self.time_offset)
                
                # ИЗМЕНЕНО: Применяем загруженные размеры к окну
                if 'fig_width' in new_s and 'fig_height' in new_s:
                    self.fig_w_var.set(new_s['fig_width'])
                    self.fig_h_var.set(new_s['fig_height'])
                    self.preset_cb.set("Свой размер")
                    self.apply_figure_size(None)
    ...
    def reset_all(self):
        ...
        self.convert_sec_to_min_var.set(False)
        self.reverse_x_var.set(False)
        
        # Сброс размеров окна
        self.fig_w_var.set(10.0)
        self.fig_h_var.set(6.0)
        self.preset_cb.set("По умолчанию (10x6)")
        self.apply_figure_size(None)
