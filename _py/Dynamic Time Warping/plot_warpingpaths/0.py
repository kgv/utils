...
# # ==========================================
# # 5. ВИЗУАЛИЗАЦИЯ ТЕПЛОВОЙ КАРТЫ (Warping Paths)
# # ==========================================
# print("Вычисление полной матрицы путей...")

# s1 = np.array(series_list[0], dtype=np.float64)
# s2 = np.array(series_list[1], dtype=np.float64)

# # Функция warping_paths возвращает итоговую дистанцию (d) и саму матрицу (paths)
# d, paths = dtw.warping_paths(s1, s2, window=WINDOW_SIZE, psi=2)

# # --- ИСПРАВЛЕНИЕ ---
# # Превращаем "список списков" в двумерный массив numpy, 
# # чтобы функция plot_warpingpaths смогла его правильно прочитать
# paths = np.array(paths)

# # Извлекаем наилучший путь из рассчитанной матрицы
# best_path = dtw.best_path(paths)

# # Строим график (тепловую карту)
# fig, ax = dtwvis.plot_warpingpaths(s1, s2, paths, best_path)

# # Задаем заголовок
# fig.suptitle(f"Матрица стоимости DTW (Окно={WINDOW_SIZE}):\n{names[0]} и {names[1]}", fontsize=14)
