...
class ChromatogramApp:
    ...
    def refresh_plots(self, force=False):
        ...
        for h in self.helper_lines:
            ...
            rot = h.get('label_rotation', 0) # ДОБАВЛЕНО: Получаем угол поворота
            ...
            if h['type'] == 'V':
                ...
                # ДОБАВЛЕНО: rotation=rot
                t = self.ax.text(tx, ty, txt, color=h['color'], fontsize=h['font_size'], 
                                 va='center', ha='center', fontweight='bold', zorder=z+1, rotation=rot)
            else:
    ...
    # ДОБАВЛЕНО: Новый метод для применения ручных значений Y
    ...
    def add_helper(self):
        ...
        new_h = {
            ...
            'label_pos': 0.5, 
            'label_offset': 0.0, 
            'label_rotation': 0,  # ДОБАВЛЕНО: Угол поворота по умолчанию
            'layer': 'Задний', 
            'font_size': 9
        }
    ...
    def edit_helper(self, idx, key, val):
        try:
            # ДОБАВЛЕНО: label_rotation в список числовых параметров
            if key in ['pos', 'start', 'end', 'label_offset', 'label_rotation']:
        ...
        try:
            ...
            if not raw_val:
                if key == 'label_pos':
                    ...
                    spinbox_widget.insert(0, str(int(self.helper_lines[idx].get(key, 0.5) * 100)))
                elif key == 'font_size':
                    spinbox_widget.delete(0, "end")
                    spinbox_widget.insert(0, str(int(self.helper_lines[idx].get(key, 9))))
                else:
                    spinbox_widget.delete(0, "end")
                    spinbox_widget.insert(0, str(self.helper_lines[idx].get(key, 0))) # ИЗМЕНЕНО: Безопасное получение значения
                return
