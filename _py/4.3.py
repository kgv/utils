...
class ChromatogramApp:
    ...
    def setup_tab_scale(self):
        ...
        # NEW: Time offset input
        offset_frame = ttk.Frame(frame); offset_frame.pack(fill="x", pady=(10, 5))
        ttk.Label(offset_frame, text="Смещение по X (мин):").pack(side="left")
    ...
    def reapply_offset_to_all_plots(self):
        # This function will re-calculate 't' for all plots based on the new offset
        # It assumes 't_raw' stores the original time data without offset
        new_all_data_bounds = [float('inf'), float('-inf')]
        for item in self.plots_registry:
            # If 't_raw' exists, use it. Otherwise, assume 't' is raw and store it.
            if 't_raw' not in item:
                item['t_raw'] = item['t'] - self.time_offset # Store current 't' as raw
            
            item['t'] = item['t_raw'] + self.time_offset
            item['line'].set_xdata(item['t']) # Update the line's x-data
        ...
        if self.all_data_bounds[0] != float('inf'):
            ...
            # Adjust scale sliders if they are outside new bounds
        ...
        self.sync_time_scale(None) # Update entry fields
    ...
    def export_style(self):
        ...
        if path:
            ...
            data['time_offset'] = self.time_offset_var.get() # Export time offset
        ...
        if path:
            try:
                ...
                if 'time_offset' in new_s:
                    ...
                    self.time_offset_var.set(self.time_offset)
                    self.reapply_offset_to_all_plots() # Reapply offset to existing plots
    ...
    def reset_all(self):
        ...
        self.reapply_offset_to_all_plots() # Reapply offset (which is now 0)
